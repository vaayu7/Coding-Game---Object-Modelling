...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
     List<Contest> allContests = contestMap.entrySet().stream().map(Map.Entry :: getValue).collect(Collectors.toList());
     return allContests;
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        List<Contest> allContestLevel= contestMap.entrySet().stream().filter(e->e.getValue().getLevel().equals(level)).
        map(Map.Entry::getValue).collect(Collectors.toList());
        
        return allContestLevel;
    }
